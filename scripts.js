...
// Make multiple Pacmen at a random position with random velocity
function makePac() {
...
function update() {
 ...
 function checkCollisions(item) {
   //detect collision with all walls and make pacman bounce  
    ...
    if ( item.position.y + item.velocity.x + item.newimg.height > window.innerHeight || item.position.y + item.velocity.y < 0) {
